...
smc-β-α : {A B C : Obj} → (⊗-β {A}{B} ⊗ₐ id {C}) ○ ⊗-α {B}{A}{C} ○ (id {B} ⊗ₐ ⊗-β {A}{C}) ≡h ⊗-α {A}{B}{C} ○ ⊗-β {A}{B ⊗ C} ○ ⊗-α {B}{C}{A}
smc-β-α {U , X , α}{V , Y , β}{W , Z , γ} = (ext-set aux₁) , ext-set aux₂
 where
   aux₁ : {a : (U × V) × W} → (⟨ id-set , twist-× ⟩ ∘ (lr-assoc-× ∘ ⟨ twist-× , id-set ⟩)) a ≡ (lr-assoc-× ∘ (twist-× ∘ lr-assoc-×)) a
   aux₁ {(u , v) , w} = refl

   aux₂ : {a : (W × U → Y) × (V → (U → Z) × (W → X))} →
        _≡_ 
          ((_∘_ 
          (_∘_ 
            (F⊗ {W} {(U → Y) × (V → X)} {V × U} {Z} {W} {(V → X) × (U → Y)} {U × V} {Z}
                {twist-× {A = U} {V}}
                {twist-× {A = U → Y} {V → X}} {id-set {A = W}}
                {id-set {A = Z}})
            (Fα {U} {W} {Y} {X} {V} {U} {Z}))
          (F⊗ {W × U} {Y} {V}
              {(U → Z) × (W → X)} {U × W} {Y}
              {V} {(W → X) × (U → Z)} {id-set {A = V}}
              {id-set {A = Y}} {twist-× {A = U} {W}}
              {twist-× {A = U → Z} {W → X}})) a)
          ((_∘_ 
          (_∘_ 
            (Fα {V} {W} {X} {Y} {U} {V} {Z})
            (twist-× {A = U → (W → Y) × (V → Z)}{V × W → X}))
          (Fα {W} {U} {Y} {Z} {V} {W} {X})) a)
   aux₂ {t₁ , t₂} = eq-× (ext-set aux₃) (ext-set (λ {a} → aux₄ {a}))
    where
      aux₃ : {a : W} →
        _≡_ {lzero} {_×_ {lzero} {lzero} (V → X) (U → Y)}
        ((λ x₁ →
            fst {lzero} {lzero} {W → X} {U → Z}
            (twist-× {lzero} {lzero} {U → Z} {W → X}
            (t₂ (id-set {lzero} {V} x₁)))
            (id-set {lzero} {W} a))
        , (λ x₁ → id-set {lzero} {Y} (t₁ (id-set {lzero} {W} a , x₁))))
        ((λ x₁ → snd {lzero} {lzero} {U → Z} {W → X} (t₂ x₁) a) ,
        (λ x₁ → t₁ (a , x₁)))
      aux₃ {w} = eq-× (ext-set aux₄) refl
       where
         aux₄ : {a : V} → fst (twist-× (t₂ (id-set a))) (id-set w) ≡ snd (t₂ a) w
         aux₄ {v} with t₂ v
         ... | r₁ , r₂ = refl

      aux₄ : {a : U × V} → id-set (snd (twist-× (t₂ (id-set (fst (twist-× a))))) (snd (twist-× a))) ≡ fst (t₂ (snd a)) (fst a)
      aux₄ {u , v} with t₂ v
      ... | r₁ , r₂ = refl

smc-β-λ : {A : Obj} → ⊗-β {A}{I} ○ ⊗-λ {A} ≡h ⊗-ρ {A}
smc-β-λ {U , X , α} = ext-set (λ {a} → aux {a}) , refl
 where
   aux : {a : U × ⊤} → (snd ∘ twist-×) a ≡ fst {A = U}{⊤ {lzero}} a
   aux {u , triv} = refl
