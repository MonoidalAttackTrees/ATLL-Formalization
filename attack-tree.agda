...
≈'-NODE : ∀{b t} → NODE b ≈' t → (t ≡ NODE b)
≈'-NODE-sym : ∀{b t} → t ≈' NODE b → (t ≡ NODE b)
≈'-NODE {b} {t} (≈'-reduce ())
≈'-NODE {b} {.(NODE b)} ≈'-refl = refl
≈'-NODE {b} {t} (≈'-sym p) = ≈'-NODE-sym p
≈'-NODE {b} {t} (≈'-trans p₁ p₂) with ≈'-NODE p₁
... | refl = ≈'-NODE p₂
≈'-NODE-sym {b} {t} (≈'-reduce ())
≈'-NODE-sym {b} {.(NODE b)} ≈'-refl = refl
≈'-NODE-sym {b} {t} (≈'-sym p) = ≈'-NODE p
≈'-NODE-sym {b} {t} (≈'-trans p₁ p₂) with ≈'-NODE-sym p₂
... | refl = ≈'-NODE-sym p₁
...
≃-refl : ∀{t} → t ≃ t
≃-refl {t} = t , (t , (⟿-refl , (⟿-refl , (t , (⟿'-refl , ⟿'-refl)))))

≃-sym : ∀{t₁ t₂} → t₁ ≃ t₂ → t₂ ≃ t₁
≃-sym {t₁}{t₂} (s₁ , s₂ , p₁ , p₂ , s₃ , p₄ , p₅) = s₂ , (s₁ , (p₂ , (p₁ , (s₃ , (p₅ , p₄)))))

≃-trans : ∀{t₁ t₂ t₃} → t₁ ≃ t₂ → t₂ ≃ t₃ → t₁ ≃ t₃
≃-trans {t₁}{t₂}{t₃} (s₁ , s₂ , p₁ , p₂ , s₃ , p₄ , p₅) (s₄ , s₅ , p₆ , p₇ , s₆ , p₈ , p₉) with CR p₂ p₆
... | (s₁' , r₁ , r₂) with CR (⟿'*-⟿* p₅) r₁
... | (s₂' , r₃ , r₄) with CR r₂ (⟿'*-⟿* p₈)
... | (s₃' , r₅ , r₆) with CR r₄ r₅
